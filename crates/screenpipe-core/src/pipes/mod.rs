...
fn render_pipe_system_prompt(
    ...
    sys.push_str(&format!(
        "CRITICAL: You ARE this pipe. You are already running inside it. NEVER run `screenpipe pipe run` — that would create a recursive duplicate. Execute the task directly using the tools available to you (bash, file I/O, HTTP requests, etc.).\n\nOS: {os}\nOutput directory: ./output/\nScreenpipe API: http://localhost:{api_port}{api_auth_note}\nPrefer bun/TypeScript for scripts. Python may not be installed.\nSend notifications via POST http://localhost:11435/notify with {{\"title\": \"...\", \"body\": \"...\"}}. Body supports markdown. File links MUST use absolute paths (e.g. [View log](/Users/me/file.md)), never relative paths like ./output/file.md — relative paths break the notification link handler.\nNotifications can carry up to 5 action buttons via `\"actions\": [...]` — USE THEM whenever a human decision or follow-up would help, instead of a passive FYI. When you find something the user might want acted on (send/share/draft/fix/dig deeper), ASK via an actionable notification rather than doing nothing or acting unilaterally. Each action: {{\"id\", \"label\", \"type\", \"primary\"?}} plus per-type fields. Types: `chat` runs an inline prompt in a fresh chat session when clicked — write the whole follow-up task in `prompt`, attach data in `context`, set `auto_send: false` to let the user review first (best default for one-off follow-ups); `pipe` runs an installed pipe (`pipe` name + optional `context`); `link`/`deeplink` open a URL; `api` POSTs a local endpoint (`url`); `dismiss` declines. Example: {{\"title\": \"draft follow-up for the acme call?\", \"body\": \"found 3 action items\", \"actions\": [{{\"id\": \"go\", \"label\": \"draft it\", \"type\": \"chat\", \"primary\": true, \"prompt\": \"draft a follow-up email for the acme call using the action items in context\", \"context\": {{\"meeting_id\": 42}}}}, {{\"id\": \"no\", \"label\": \"skip\", \"type\": \"dismiss\"}}]}}. Unclicked actions stay available in the notification bell, so asking is cheap.\n\n"
    ));
    sys.push_str(body);
