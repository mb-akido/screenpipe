...
Keep the staged hosted preview off until its `ask_user` continuation policy is explicit and tested. With a two-turn lifetime allowance, an `ask_user` request emitted on turn two cannot unlock a form whose answer would silently require a third paid turn. The enablement-safe choices are to omit `ask_user` from free-preview tools or bind the answer to the already-reserved parent turn with its own bounded continuation lease. The current desktop persistence work only guarantees duplicate-safe fail-closed delivery across queueing, remounts, provider changes, cancellation, and deletion; it does not by itself choose that product/accounting policy.
...
| Native accepts an Ask User answer or retry while the visible chat switches provider | The same conversation stays visibly accepted/locked and the captured file is committed once; a different account/session panel is never mutated |
| Queued Ask User answer is accepted, provider switches, and native cancellation loses because work already started | The pending marker remains locked in React, store, and disk; persistence retries and cancellation cannot unlock a duplicate submission |
| WebView remount loses ephemeral queued-display state before an Ask User reply starts | The persisted reply text recovers the owning tool id on `user_start`; the materialized row is accepted and the card cannot enqueue it again |
| Chat deletion races an initial save, queued send, steering call, crash restart, or cross-window delayed save | The deletion tombstone blocks native dispatch and file/store resurrection; the pre-delete broadcast propagates that boundary to every WebView |
...
1. Keep both flags `false` and confirm no product claim says the free-tier enforcement is already live.
2. Validate the CREATE-only production bootstrap in `packages/ai-gateway/migrations-free-chat/` with a fresh apply, reapply, and schema preflight; separately validate the historical chain on a fresh local-only database. The deployment workflow applies only the dedicated bootstrap, recorded in `free_chat_migrations`, before deploying the Worker, so code and its scheduled cleanup cannot arrive ahead of the ledger schema.
3. Deploy and verify the companion website entitlement changes first. The gateway deliberately trusts the canonical per-feature `/api/user` result, not a raw subscription row; enforcing hosted routes before that resolver is live could misclassify paying or canceled users.
4. Establish the durable server baseline before releasing Free / Local desktop access: set `FREE_LOCAL_TIER_ENFORCEMENT_ENABLED=true` and keep `FREE_CHAT_PREVIEW_ENABLED=false`. Verify signed-out, authenticated free, Business/enterprise, old-client, local/BYOK, background, transcription, and alternate hosted routes. This closes unlimited Screenpipe-funded inference before the new free shell reaches users.
5. Ship the desktop build with turn markers, the `426` update-required path, sign-in handling, provider choice, and the structured limit modal while the preview remains off. This is the safe tomorrow-launch state: Free / Local plus unlimited user-funded AI, with no included hosted inference yet.
6. Before opening the preview, choose and test the `ask_user` continuation policy described above, require provider-credit depletion alerts, provider budget alerts, D1/cron health alerts, and an operator-confirmed emergency-stop procedure. After the minimum compatible desktop version has sufficient adoption, set `FREE_CHAT_PREVIEW_ENABLED=true` globally and verify the two-turn ledger plus every circuit breaker. There is no partial rollout mechanism in this contract.
7. Watch accepted/failed turns, calls and requests per turn, output tokens, network/global/shadow stops, shadow reserved dollars, `426` rates by app version, D1 errors, actual cash spend, provider-credit balance/budget alerts, provider choice, checkout conversion, and local recording retention.

**Never run `packages/ai-gateway/migrations/` or `wrangler.historical-migrations.toml` against production.** Production already has the historical usage and cost schema without a matching `d1_migrations` baseline; replaying it would repeat `ALTER TABLE` statements and attempt the known-expensive `cost_log` index against a multi-gigabyte table. The historical config exists only to prove the full chain on a fresh local CI database. Production uses `packages/ai-gateway/wrangler.toml`, the isolated `migrations-free-chat/` directory, and the separate `free_chat_migrations` table.

`packages/ai-gateway/wrangler.toml` uses `keep_vars = true`, so routine Worker deploys preserve dashboard-managed rollout variables that are omitted from the file. Before deploying, the workflow rejects repository variables that request `preview=true` without `enforcement=true`, applies only the production-safe free-chat bootstrap, verifies the remote columns and indexes, and only then publishes the Worker. After deploying, it reads `/test`, independently rejects that dangerous live combination, and compares `X-Free-Local-Tier-Enforcement` and `X-Free-Chat-Preview` with `FREE_LOCAL_TIER_ENFORCEMENT_EXPECTED` and `FREE_CHAT_PREVIEW_EXPECTED`.
