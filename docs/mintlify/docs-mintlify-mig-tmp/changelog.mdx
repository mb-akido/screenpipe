...
- **VS Code terminal capture** — screenpipe now captures the contents of the integrated terminal in VS Code and its forks (Cursor, Windsurf, Antigravity, VSCodium, Positron, Void, Aide, Trae). The active window name also reflects the terminal session label (branch, directory, or custom tab name) when the terminal panel is focused, so AI context distinguishes terminal activity from editor activity. see [privacy filter](/privacy-filter).
- **Workflowy connection** — connect Workflowy from **settings → connections** to let pipes and chat append bullets to a Workflowy outline. see [connections](/connections) and [connection reference](/connection-reference).
- **multi-account Google Docs and Sheets** — Google Docs and Sheets now support multiple connected accounts with per-account disconnect and an "add another account" flow, matching Gmail. Reconnecting Google or Microsoft 365 also forces the account picker, so adding a second account no longer silently re-consents the first. see [connections](/connections).
- **Cmd+N / Ctrl+N for a new chat** — open a new chat from the home page with Cmd+N (macOS) or Ctrl+N (Windows/Linux); the chat input auto-focuses so you can start typing immediately.
- **Ctrl+Tab recent chat switcher** — switch between recent chats with Ctrl+Tab, the same way you alternate between browser tabs.
- **simpler onboarding pipe step** — the final onboarding step now shows a pre-checked list of recommended pipes with a single "install" button and a "customize" expand for the rest, replacing the previous three-path picker. Skipping still installs the most-used pipe so new users land with a useful agent enabled.

### updates

- **cleaner pipes page** — the pipes page has been consolidated into a single toolbar (search · type · favorites · refresh). The integration icons strip, "this device" dropdown, and stacked filter tab rows are gone, and the team filter only appears when you actually have team-shared pipes. see [pipes](/pipes).
- **fewer duplicate source citations in chat** — agentic chats and pipe runs that touch the same files across many steps now show a single aggregated "Sources" footer per turn instead of one footer under every assistant message. Markdown exports of those chats are deduped the same way.
- **clearer multi-account errors from connections** — when the AI calls Gmail / Calendar / Docs / Sheets without picking an instance and you have multiple accounts connected, the error now tells the AI exactly which accounts exist and how to specify one, instead of falsely reporting "not connected". The chat surface also labels connection actions by what they did ("Email sent", "Doc created", "N cells appended") rather than a generic "Configured gmail connection". see [connections](/connections).

### bug fixes

- **Pi assistant no longer 400s on Vertex / DeepSeek** — fixed two AI-gateway bugs that caused Pi requests to fail with "No tool calls but found tool output" on Vertex MaaS DeepSeek and similar providers. Tool calls now round-trip cleanly through the proxy.
- **Pi cloud auth refreshes at runtime** — the Pi assistant now refreshes its cloud auth token when it expires instead of getting stuck on stale anonymous-tier 403 errors.
- **Intel Mac Pi install fixed** — the Pi background installer no longer fails with "Bad CPU type in executable" on Intel Macs; the bundled `bun` sidecar is now built for the right architecture.
- **first-launch boot crash after auto-update** — fixed the "something went wrong" error boundary that hit on the first launch after some auto-updates.
- **scheduled pipes don't fire on install** — a newly installed scheduled pipe (e.g. "every day at 7am") no longer runs immediately on install. The scheduler also no longer plays catch-up after the app was off across a slot — it waits for the next real scheduled time. see [pipes](/pipes).
- **scheduled pipes can reach the local API again** — fixed a regression where pipes spawned by the scheduler could 403 against the local proxy because the auth env var wasn't propagated to the child process. see [pipes](/pipes).
- **HTTP MCP sessions register correctly** — `npx screenpipe-mcp --http` sessions are now registered after `initialize`, so HTTP MCP clients reconnect reliably. see [MCP server](/mcp-server).
- **chat preset stops snapping back to default** — picking a non-default chat preset (e.g. Ollama "high") no longer silently reverts to the default preset after an unrelated settings write. The next message is stamped with the model you actually chose.
- **`/search` accepts numeric and yes/no booleans** — `?include_frames=1`, `?focused=yes`, and an empty `?focused=` no longer return a cryptic 400. All five boolean filters on `/search` accept `true`/`false`/`1`/`0`/`yes`/`no`/`on`/`off`. see the [API docs](/for-developers).
- **excluded apps actually filtered on Windows** — apps listed in `ignored_windows` are now properly excluded from capture on Windows, including WebView2 and shell-hosted Chromium variants. see [privacy filter](/privacy-filter).
- **expanded shortcut pill buttons click again** — timeline, chat, search, meeting toggle, and the close (X) inside the expanded shortcut pill now respond to clicks, while drag-to-move still works.
- **local media renders reliably in chat** — local Screenpipe recording paths in chat now play back as inline media across macOS, Linux, and Windows, including audio-only MP4 chunks (played as audio, not silent video).
- **macOS Low Power Mode no longer kills capture** — unplugging at 100% battery used to silently pause capture because macOS auto-enables Low Power Mode on unplug. Low Power Mode now drops to the Saver profile instead of full pause; only critical battery (≤10%) triggers a full pause. The tray menu also accurately shows "Paused" when capture is paused. see [privacy data flow](/privacy-data-flow).
- **Recording settings page no longer blanks on low battery** — fixed a missing icon mapping that crashed the Recording settings page when the power profile dropped to `audio_paused` or `full_pause`.
- **desktop login URL fixed** — the "Sign in" link from the desktop app now opens the correct login URL.
- **chat tool-call counter polish** — completed tool groups no longer show a redundant `[N]` count next to summaries like "ran 1 command" or "read 3 files"; the count only shows while the group is running.
- **VS Code terminal window naming polish** — fixed terminal window labels that included xterm.js accessibility hint text or fell back to the open editor file when the terminal was focused.

### chores

- **Intercom live chat removed from Settings** — the embedded "Live chat with us" panel has been removed from the in-app feedback section. Discord, GitHub, the founder calendar, and email remain as support channels.
