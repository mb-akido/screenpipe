...
fn msvc_runtime_library(static_crt: bool) -> &'static str {
    if static_crt {
...
fn main() {
    ...
    if target.contains("windows-msvc") {
        ...
        // cmake::Config::static_crt() is not enough for this dependency tree on
        // every Windows host: some users still end up with knf object files
        // compiled against /MT, which then fail to link against ONNX Runtime's
        // /MD import library (LNK2038 RuntimeLibrary mismatch). Force the CMake
        // runtime selection explicitly so both the top-level knfc target and the
        // nested kaldi-native-fbank target agree on the CRT.
        config.define("CMAKE_POLICY_DEFAULT_CMP0091", "NEW");
        config.define(
            "CMAKE_MSVC_RUNTIME_LIBRARY",
            msvc_runtime_library(static_crt),
        );
