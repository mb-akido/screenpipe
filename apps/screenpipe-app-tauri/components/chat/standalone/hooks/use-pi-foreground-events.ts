        ...
        if (
          ...
          if (quotaErrorType === "daily" || quotaErrorType === "rate") {
            ...
            if (piMessageIdRef.current) {
              const msgId = piMessageIdRef.current;
              setMessages((prev) =>
                prev.map((m) => m.id === msgId ? { ...m, content: "This model requires an upgrade to Screenpipe Business. Switch to Auto or Claude Haiku to keep going." } : m)
              );
            }
          } else {
          ...
          if (piMessageIdRef.current && !isPipeWatch) {
            ...
            if (agentEndError && !content) {
              ...
              if (isInvalidatedAuthTokenError(errStr)) {
                ...
                content = "This model requires an upgrade to Screenpipe Business. Switch to Auto or Claude Haiku to keep going.";
              } else {
