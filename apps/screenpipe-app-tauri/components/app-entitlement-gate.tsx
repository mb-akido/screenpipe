...
export function AppEntitlementGate({ children }: { children: React.ReactNode }) {
  ...
  const useDifferentAccount = useCallback(async () => {
    try {
      const result = await commands.setCloudToken(null, user?.token ?? null, true);
      if (result?.status === "error") {
        console.warn("cloud token persistence failed after runtime account clear:", result.error);
        setRefreshError("couldn't switch accounts safely; please try again");
        return;
      }
    } catch (e) {
    ...
    try {
      ...
      if (result?.status === "error") {
        console.warn("failed to clear pi config before switching accounts:", result.error);
        setRefreshError("your account changed while switching; please try again");
        return;
      }
    } catch (e) {
