...
/// Restart just the capture session (vision + audio managers) in place, leaving
/// the HTTP server alive. Driven by the health watchdog to recover from a silent
/// capture-stream freeze — e.g. macOS ScreenCaptureKit stops delivering frames,
/// Windows WGC stalls, or the capture loop is alive but DB writes are blocked —
/// where the app still reports "recording" yet no new frames/audio are written.
///
/// This is the lightweight tray-toggle path (`stop_capture` + `start_capture`),
/// NOT a full `stop_screenpipe`/`spawn_screenpipe` cycle: re-creating the
/// `CaptureSession` reacquires the OS capture streams, which is what unwedges a
/// frozen SCK/WGC stream. If the server itself is unresponsive, `start_capture`
/// escalates to a full restart via the `request-server-restart` event.
pub async fn auto_restart_capture(app: tauri::AppHandle) -> Result<(), String> {
    // Don't fight a restart/start that's already in flight (manual toggle,
    // settings-apply, mic-grant reinit). They'll establish a fresh session.
    {
        let state = app.state::<RecordingState>();
        if state.is_starting.load(Ordering::SeqCst)
            || state.is_starting_capture.load(Ordering::SeqCst)
        {
            info!("auto_restart_capture: a start is already in progress, skipping");
            return Ok(());
        }
    }

    // Tear down the (frozen) capture session if one exists.
    {
        let state = app.state::<RecordingState>();
        let running = state.capture.lock().await.is_some();
        if running {
            if let Err(e) = stop_capture(state, app.clone()).await {
                warn!("auto_restart_capture: stop_capture failed: {}", e);
            }
        }
    }

    // Give the OS a beat to release the capture stream before reacquiring.
    tokio::time::sleep(Duration::from_millis(500)).await;

    // Retry start_capture with backoff: "Server not running/responding" and a
    // requested full restart are transient while the backend reboots.
    const MAX_ATTEMPTS: u32 = 10;
    const BACKOFF_MS: u64 = 500;
    let mut last_err: Option<String> = None;
    for attempt in 1..=MAX_ATTEMPTS {
        let state = app.state::<RecordingState>();
        match start_capture(state, app.clone()).await {
            Ok(()) => {
                info!(
                    "auto_restart_capture: capture restarted (attempt {}/{})",
            ...
            Err(e) => {
                let transient = e.contains("Server not running")
                    ...
                    || e.contains("full restart requested");
                if !transient {
                    warn!("auto_restart_capture: start_capture failed: {}", e);
                    return Err(e);
                }
                last_err = Some(e);
    ...
    warn!(
        "auto_restart_capture: gave up after {} attempts ({}s): {}",
        MAX_ATTEMPTS,
