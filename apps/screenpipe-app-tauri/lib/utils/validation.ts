...
export const settingsStoreSchema = z.object({
  ...
  visionIndexingMode: z.enum(["off", "local", "cloud"]).optional(),
  visionIndexingContextMode: z.enum(["augment", "replace"]).optional(),
  visionIndexingEndpoint: z.string().optional().nullable(),
  visionIndexingApiKey: z.string().optional().nullable(),
  visionIndexingModel: z.string().optional().nullable(),
  visionIndexingIntervalMs: z.number().int().min(10000).max(300000).optional(),
  visionIndexingCloudConsent: z.boolean().optional(),
  visionIndexingMaxCloudJobsPerDay: z.number().int().min(1).max(100000).optional(),
